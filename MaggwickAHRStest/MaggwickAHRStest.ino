...
void loop(){
    ...
    MadgwickAHRSupdateIMU(event.gyro.x,event.gyro.y,event.gyro.z,event.acceleration.x,event.acceleration.y,event.acceleration.z,dt);
    toEulerianAngle();
...
void MadgwickAHRSupdateIMU(float gx, float gy, float gz, float ax, float ay, float az, float _dt) {
	float recipNorm;
	float s0, s1, s2, s3;
	...
	q0 += qDot1 * _dt;//(1.0f / sampleFreq);
	q1 += qDot2 * _dt;//(1.0f / sampleFreq);
	q2 += qDot3 * _dt;//(1.0f / sampleFreq);
	q3 += qDot4 * _dt;//(1.0f / sampleFreq);
...
{
	double ysqr = q1 * q1;
	double t0 = -2.0f * (ysqr + q2 * q2) + 1.0f;
	double t1 = +2.0f * (q0 * q1 - q3 * q2);
	double t2 = -2.0f * (q0 * q2 + q3 * q1);
	double t3 = +2.0f * (q1 * q2 - q3 * q0);
	double t4 = -2.0f * (q0 * q0 + ysqr) + 1.0f;
